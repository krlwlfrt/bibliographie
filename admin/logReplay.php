...
if($logCount_file > $logCount_database){
	...
	switch($_GET['task']){
		case 'replay':
				...
				try {
					...
					if($row->category == 'authors'){
						...
						elseif($row->action == 'editTopic'){
							$topic = bibliographie_topics_get_data($data->dataBefore->topic_id);
							$topic->topics = bibliographie_topics_get_parent_topics($topic->topic_id);
							$dataAfter = $data->dataAfter;
			...
			if($logCount_file == $row->id and $result !== false)
				echo '<p class="success">All changes have been applied. You can go back <a href="'.BIBLIOGRAPHIE_WEB_ROOT.'">using bibliographie</a> now!</p>';
			break;
